...
import { Follower, UserEntity } from '../../../entities/user-entity';
...
export function useFollowers() {
    async function getFollower() {
        const response = await apiV1.get<null, { data: Follower[] }>(
            'followers', {
            headers: {
                Authorization: `Bearer ${Cookies.get("token")}`
            }
    ...
    const { data, isLoading } = useQuery<Follower[], Error, Follower[]>({
        queryKey: ['followers'],
        queryFn: getFollower,
    });
